...
In this example, we will use one of the many `np.Templating` module methods, <Link href="templating-modules-web">Templating Web Module.</Link>.
...
The call to `<%- web.weather() %>` will use the format you have defined in `np.Templating` <Link href="templating-settings">settings</Link>. In this example, the settings value is blank, thus the default weather format will be used.

For more information about `np.Templating` weather, visit <Link href="templating-modules-web">Templating Web Module</Link>.
