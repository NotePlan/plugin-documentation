...
<Callout
  type="info"
  description={`
<code>np.Templating</code> doesn't escape characters by default. When doing web requests, it may be useful to escape dangerous characters. You can escape a command's response characters using the <code><%-</code> tag.
