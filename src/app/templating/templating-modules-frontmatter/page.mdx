...
The FrontMatter Module exposes commonly used <a href="templating-modules-helpers"> helper </a> methods
...
<Callout
  type="info">
When implementing <code>NPTemplating.renderTemplate</code> if template is a valid frontmatter template, they will be rendered automatically without performing any additional frontmatter processing.

  </Callout>

<Callout
  type="warning"
  title="INVALID FRONTMATTER VALUES"
  description={`
...
_Note: Also available as <Link href="templates-modules-helpers">helper</Link>
method._
...
If you wish to display prompts in your frontmatter attributes section, you can just use the standard `Templating` prompt command

### Example

Using something like the following, when the template is rendered `Templating` will display prompt requesting `newNoteTitle`
...
When using any of the following <code>Templating</code> commands (<code>np:new</code>, <code>np:qtn</code>), the created note will use the <code>newNoteTitle</code> internally
  `}
/>
