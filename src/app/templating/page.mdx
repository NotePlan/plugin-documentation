...
`Templating` has a number of <Link href="templating-settings" alt="settings">settings</Link> which you can use to control how data is supplied and formatted when rendering templates.

<Callout
  type="info"
  description={`
For more information on <code>Templating</code> settings, visit <a href="templating-settings" alt="settings">settings</a>.
  `}
/>
## Using Templating Commands

`Templating` provides a variety of built-in commands which can be invoked using NotePlan Plugin Command Bar (`Command-J`)
...
You can also use the Templating functions from within a custom plugin you are developing. For example:
