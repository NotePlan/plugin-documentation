...
`Templating` provides support for templates which create notes with properties/frontmatter of their own. The note properties/frontmatter used by the template processor is placed at the top in the Properties editor (which is between `--` separators in the raw file). 

You can add properties for **the new note's frontmatter** by enclosing it at the very top of the template between the `--` (two dashes) separators.
...
<DynamicImage src="note-frontmatter.png" alt="Note Frontmatter Example" width="70%" />
