...
- `helpers.chooseNote()` - Select a note (dropdown displays as `<path>/<title>` for personal/project notes)
...
**Note:** When using `helpers.chooseNote()`, the dropdown options for personal/project notes display in the format `<path>/<title>` (e.g., `Projects/Meeting Notes`), not `<title><path>`. This format makes it easier to identify notes by their location in your folder structure.
