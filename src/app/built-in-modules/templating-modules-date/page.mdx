...
<Callout type="info" title="Enhanced Date Support">
This method now supports formatting specific dates in addition to offset calculations. This resolves issues where templates needed to format dates from variables like `Editor.title`.
</Callout>

### `now(format?: string, dateOrOffset?: string | number)`
Returns the current date and time, a date/time offset from now, or formats a specific date, formatted as a string.
...
*   `dateOrOffset` (optional): A date string, offset string, or number.
    *   **Date String**: If a date string is provided (e.g., `'2025-08-06'`, `'August 6, 2025'`, `'08/06/2025'`), it formats that specific date.
        *   Supported formats: `YYYY-MM-DD`, `MM/DD/YYYY`, `DD/MM/YYYY`, month names (e.g., `'August 6, 2025'`), ISO datetime strings.
        *   Examples: `'2025-08-06'` (August 6, 2025), `'December 25, 2024'` (Christmas 2024).
    ...
    *   If empty or not provided, the current date is used.
...
*   **Date vs Offset Detection**: The method automatically detects whether the second parameter is a date string or an offset:
    *   Date strings are identified by patterns like `YYYY-MM-DD`, `MM/DD/YYYY`, month names, or ISO datetime strings.
    *   Single numbers or shorthand strings (e.g., `7`, `1d`, `10w`) are treated as offsets.
    *   If a string doesn't match date patterns, it's treated as an offset.
...
The following example formats a specific date using the default format:
```javascript
<%- date.now('', '2025-08-06') %>
```
Output: `2025-08-06`

The following example formats a specific date with a custom format:
```javascript
<%- date.now('YYYY - dddd', '2025-08-06') %>
```
Output: `2025 - Wednesday`

The following example formats a date using month name format:
```javascript
<%- date.now('MMMM Do, YYYY', 'August 6, 2025') %>
```
Output: `August 6th, 2025`

The following example formats a date using MM/DD/YYYY format:
```javascript
<%- date.now('YYYY-MM-DD', '08/06/2025') %>
```
Output: `2025-08-06`

The following example uses the global `now()` helper to format a specific date:
```javascript
<%- now('dddd, MMMM Do', '2025-08-06') %>
```
Output: `Wednesday, August 6th` 
