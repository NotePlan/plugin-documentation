...
>
> Returns service information from custom URL or entry from `Templating` <Link href="templating-settings">Web Service Settings</Link>
...
>
> Returns weather information from <a href="https://wttr.in" alt="wttr.in">wttr.in </a> weather service, using the <Link href="templating-settings">Web Service Settings</Link> from `Templating` Settings, or a custom `format` string.

<Callout
  type="info"
  description={`
You can set the default weather format in <code>Templating</code> <a href="templating-settings" alt="settings">settings</a> which will be used when <code><%- web.weather() %></code> command is executed. If you need to specify location manually, see <a href="templating-examples-js-weather" alt="js weather example">Javascript Weather Example.
