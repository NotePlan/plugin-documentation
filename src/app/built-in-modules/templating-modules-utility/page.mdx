...
The following are the methods available in the Utility Module, and they can be used in any `Templating` template, no additional configuration.
...
Format strings may contain replacement fields rather than positional placeholders. Instead of referring to a certain argument, you can now refer to a certain key within an object. Replacement fields are surrounded by rounded parentheses - ( and ) - and begin with a keyword that refers to a key defined in an object in the second parameter:

```javascript
<%- utility.format('Hello %(name)s', { name: 'Mike' }) %>
// Hello Mike
```
...
// concatenates first and last values from Templating settings
```
