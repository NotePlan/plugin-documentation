...
`np:new` will show a list of all templates in the `Templates` folder, then create a new project note using supplied note name. You can specify a folder to create the new note in using the `folder` attribute and a new note title using the `newNoteTitle` attribute.

_aliases: npn, new, nn_

### Calling /New Note from Template from Apple/Siri Shortcuts

You can also call this command from Shortcuts using the following parameters:
  1. Template Name
  2. Folder
  3. New Note Title
...
<Callout
  type="warning"
  description={`
Note that if you first create a new note and then try to insert a template using \`insert\` or \`append\`, any folder information will be ignored because the note has already been created in the default folder or wherever you created it. If you want the folder attribute to be used, you must first create the note using the \`/New Note from Template\` command.
  `}
/>
