...
- **`replaceHeadingAndContents: true`** - Replaces the entire note including title, frontmatter, and all content
- **`replaceHeadingAndContents: false`** (default) - Only replaces the note content, preserving the title and frontmatter
...
getNoteTitled: My Tasks Document Title
replaceNoteContents: true
replaceHeadingAndContents: true
---
...
**Note:** The `replaceHeadingAndContents` flag only applies when `replaceNoteContents` is set to `true`.

### writeUnderHeading

The `writeUnderHeading` attribute tells Template Runner _under which heading_ to place the template content if you are not replacing all the note's contents. By default, content will be prepended (immediately following the heading). If a heading that you specify here does not exist in the _getNotetitled_ note, the heading and content will be created and added to the end of the note (but you can change that behavior with the `addHeadingLocation` attribute -- see below). The `writeUnderHeading` attribute should not contain any #'s (just the heading name), and the search for the heading will look for any heading which **contains** the specified heading name. This gives you flexibility to add text to the end of the heading using the `replaceHeadingWithContents` attribute -- see below, but still find the heading you want to add to.
...
- **`replaceHeadingWithContents: true`** - Replaces both the heading and its content
- **`replaceHeadingWithContents: false`** (default) - Only replaces the content under the heading, keeping the original heading

**Example replaceHeadingWithContents template:**
...
location: replace
replaceHeadingWithContents: true
---
...
**Note:** The `location: replace` and `replaceHeadingWithContents: true` combination is required for this functionality to work.
...
- **`replaceHeadingAndContents`**: Set to `true` to replace entire note including title and frontmatter when using `replaceNoteContents: true`
- **`replaceHeadingWithContents`**: Set to `true` to replace both heading and content when using `location: replace`
