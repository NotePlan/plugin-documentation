...
The `location` attribute tells Template Runner where/how to place the template contents in the note (or in the block under a heading if you are using `writeUnderHeading`). _location_ applies contextually depending on whether _writeUnderHeading_ is set or not. It may be one of the following values:
...
### Other variables (you can add as many arbitrary variables as you want)
