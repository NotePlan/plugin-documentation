...
<Callout>
IMPORTANT NOTE: writeUnderHeading uses NotePlan's definition of a block, replacing all text from the header specified until another header of the same or greater value. So if your heading is a ## level and you have ### level headings directly under it, those items will be replaced. Make sure to have a heading of level ## or # underneath. If you want, you can use the @Stacey trick and place a line with just a blank ` # `, which will be treated as an H1 when the replacer hits it and nothing beneath that line will be overwritten.
</Callout>
...
`noteplan://x-callback-url/runPlugin?pluginID=np.Templating&command=templateRunner`: This tells NotePlan to call the templateRunner command inside of the Templating plugin
`&`
